...
void CreateFracture_1(Atom3D* as, Area3d area, Array<Bond3Dv2>& bonds, double diameter,double orientation)
{
    Vector3D center(0.5,0.5,0.5);
    Vector3D normal(0,0,0);
	normal.z = cos(orientation);
	normal.x = sin(orientation);
	/*if (orientation == 1){
		normal.x = 1;
...
void Modeling3D_14()
{
	...
	<< "n_layers = "<<n_layers << " " << "Part = "<<part_third_centr << " " << "Number_crack = "<< Number_crack<< " " 
		<< "orientation = "<<orientation<< " "
	<<"parallel = " <<parallel<< " " <<"tt = "<< tt<< "\n";
	//system("pause");
	/*=====================================*/
	...
	if ((Number_crack>0.5)&&(Number_crack<2.5)){
		CreateFracture_1(as, area, bonds, diameter,orientation);      // создание 1 трещины
 ...
 /*   if (continued)   {
	...
	for (s=s0; s<=s_max; s++)
    {
		...
		if ((s % sexit == 0))//||(s<20)) 
		{
			...
			if (tt<0.5){
				area.ExtendY(as, n, 1-2*0.000001*0.5);
				std::cout << "with shear\n";
			}
			else if (tt <1.5)
			{
        ...
        Step3D(as, n, area, bonds.GetData(), N_B, a_bonds.GetData(), N_AB, press);
    }
}
